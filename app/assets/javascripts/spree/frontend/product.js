...
Spree.ready(function () {
  ...
  if (addToCartForm) {
    ...
    addToCartForm.addEventListener('submit', function (event) {
      ...
      var options = {}
      for (let input of document.querySelectorAll('#cart-form [name^="options["]')) {
        var regexp = /\[([^\[\]]*)\]/g
        var parent = options
        var res, attr = ''

        while (res = regexp.exec(input.name)) {
          if (attr) {
            if (!parent[attr]) parent[attr] = res[1] ?  {} : []
            parent = parent[attr]
          }
          attr = res[1]
        }
